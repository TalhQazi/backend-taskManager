...
const LegalCalendar = require("../models/LegalCalendar");
...
async function generateNumber() {
  const count = await LegalCalendar.countDocuments();
  const year = new Date().getFullYear();
  return `CALENDAR-${year}-${String(count + 1).padStart(4, "0")}`;
}
...
router.get("/", requireAuth, async (req, res, next) => {
  try {
    const items = await LegalCalendar.find().sort({ createdAt: -1 }).lean();
...
router.get("/:id", requireAuth, async (req, res, next) => {
  try {
    const item = await LegalCalendar.findById(req.params.id).lean();
...
router.post("/", requireAuth, requireRole(["super-admin", "admin"]), async (req, res, next) => {
  try {
    ...
    const calendarNumber = await generateNumber();
    const created = await LegalCalendar.create({ ...parsed.data, calendarNumber, createdBy: req.user?.id });
...
router.put("/:id", requireAuth, requireRole(["super-admin", "admin"]), async (req, res, next) => {
  try {
    ...
    if (!parsed.success) return res.status(400).json({ error: { message: parsed.error.errors[0]?.message || "Invalid payload" } });
    const updated = await LegalCalendar.findByIdAndUpdate(req.params.id, { ...parsed.data, updatedBy: req.user?.id }, { new: true }).lean();
...
router.delete("/:id", requireAuth, requireRole(["super-admin", "admin"]), async (req, res, next) => {
  try {
    const deleted = await LegalCalendar.findByIdAndDelete(req.params.id).lean();
