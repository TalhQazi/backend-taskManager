...
const recordingUpload = multer({
  ...
  fileFilter: (_req, file, cb) => {
    ...
    const okByMime =
      ...
      mime === "";
    const okByExt = /\.(webm|mp4|mkv|mov|ogg|wav|mp3|m4a)$/i.test(name);
    const ok = okByMime || okByExt;
    if (!ok) {
      return cb(new Error("Only video/audio recordings are allowed"));
    }
    cb(null, true);
  },
});
...
router.get("/", requireAuth, async (req, res, next) => {
  try {
    ...
    if (q && typeof q === "string") {
      ...
      andClauses.push({
        $or: [
          ...
          { roomCode: rx },
          { hostName: rx },
          { "invitedParticipants.name": rx },
          { "joinedParticipants.name": rx },
        ],
      });
    }

    // Role-based visibility: employees see meetings they host, are invited to, joined, or instant public rooms
    ...
    if (!isPrivileged && userId) {
      ...
      if (type !== "past") {
        visibilityOr.push({ meetingType: "instant", status: "active" });
      }
...
// POST /api/meetings/code/:code/recording - upload a meeting recording
router.post(
  "/code/:code/recording",
  requireAuth,
  ...
  async (req, res, next) => {
    try {
      ...
      if (!req.file) {
        return res.status(400).json({ error: { message: "Recording file is required" } });
      }

      let meeting = await Meeting.findOne({ roomCode: cleanCode });
      if (!meeting) {
        // Allow recording even for ad-hoc rooms created via join
        meeting = await Meeting.create({
          roomCode: cleanCode,
          title: `Room ${cleanCode}`,
          meetingType: "instant",
          hostId: String(req.user?.sub || req.user?.id || "guest"),
          hostName: String(req.user?.name || "Host"),
          hostEmail: String(req.user?.username || ""),
          hostRole: String(req.user?.role || "employee"),
          status: "active",
        });
      }
      ...
      const originalName =
        ...
        `meeting-${cleanCode}-${Date.now()}.webm`;

      const url = await uploadToS3(
        req.file.buffer,
        originalName.endsWith(".webm") || originalName.endsWith(".mp4")
          ? originalName
          : `${originalName.replace(/\.[^.]+$/, "") || "meeting-recording"}.webm`,
      ...
      const entry = {
        ...
        recordedBy: String(req.user?.sub || req.user?.id || ""),
        recordedByName: String(req.user?.name || req.user?.username || "Host"),
        createdAt: new Date(),
      };

      if (!Array.isArray(meeting.recordings)) meeting.recordings = [];
