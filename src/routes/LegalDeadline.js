...
async function generateNumber() {
  const count = await LegalDeadline.countDocuments();
  const year = new Date().getFullYear();
  return `DEADLINE-${year}-${String(count + 1).padStart(4, "0")}`;
}
...
router.get("/", requireAuth, async (req, res, next) => {
  try {
    const items = await LegalDeadline.find().sort({ createdAt: -1 }).lean();
...
router.get("/:id", requireAuth, async (req, res, next) => {
  try {
    const item = await LegalDeadline.findById(req.params.id).lean();
...
router.post("/", requireAuth, requireRole(["super-admin", "admin"]), async (req, res, next) => {
  try {
    ...
    const deadlineNumber = await generateNumber();
    const created = await LegalDeadline.create({ ...parsed.data, deadlineNumber, createdBy: req.user?.id });
...
router.put("/:id", requireAuth, requireRole(["super-admin", "admin"]), async (req, res, next) => {
  try {
    ...
    if (!parsed.success) return res.status(400).json({ error: { message: parsed.error.errors[0]?.message || "Invalid payload" } });
    const updated = await LegalDeadline.findByIdAndUpdate(req.params.id, { ...parsed.data, updatedBy: req.user?.id }, { new: true }).lean();
...
router.delete("/:id", requireAuth, requireRole(["super-admin", "admin"]), async (req, res, next) => {
  try {
    const deleted = await LegalDeadline.findByIdAndDelete(req.params.id).lean();
