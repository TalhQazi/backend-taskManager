...
const LegalNote = require("../models/LegalNote");
...
async function generateNumber() {
  const count = await LegalNote.countDocuments();
  const year = new Date().getFullYear();
  return `NOTE-${year}-${String(count + 1).padStart(4, "0")}`;
}
...
router.get("/", requireAuth, async (req, res, next) => {
  try {
    const items = await LegalNote.find().sort({ createdAt: -1 }).lean();
...
router.get("/:id", requireAuth, async (req, res, next) => {
  try {
    const item = await LegalNote.findById(req.params.id).lean();
...
router.post("/", requireAuth, requireRole(["super-admin", "admin"]), async (req, res, next) => {
  try {
    ...
    const noteNumber = await generateNumber();
    const created = await LegalNote.create({ ...parsed.data, noteNumber, createdBy: req.user?.id });
...
router.put("/:id", requireAuth, requireRole(["super-admin", "admin"]), async (req, res, next) => {
  try {
    ...
    if (!parsed.success) return res.status(400).json({ error: { message: parsed.error.errors[0]?.message || "Invalid payload" } });
    const updated = await LegalNote.findByIdAndUpdate(req.params.id, { ...parsed.data, updatedBy: req.user?.id }, { new: true }).lean();
...
router.delete("/:id", requireAuth, requireRole(["super-admin", "admin"]), async (req, res, next) => {
  try {
    const deleted = await LegalNote.findByIdAndDelete(req.params.id).lean();
